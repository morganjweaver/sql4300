...
std::string executeDrop(const hsql::DropStatement *stmt) {
    std::string  ret("DROP ");
    switch(stmt->type) {
...
std::string executeShow(const hsql::ShowStatement *stmt) {
    std::string ret("SHOW ");
    switch (stmt->type) {
...
std::string execute(const hsql::SQLStatement *stmt) {
    switch (stmt->type()) {
        ...
        case hsql::kStmtDrop:
            return executeDrop((const hsql::DropStatement *) stmt);
        case hsql::kStmtShow:
            return executeShow((const hsql::ShowStatement *) stmt);
